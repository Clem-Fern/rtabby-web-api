...
async fn home(
    ...
    let login_url = format!( "https://github.com/login/oauth/authorize?client_id={}&redirect_uri={}://{}/login/github/callback&state={}", client_id, scheme, req.connection_info().host(), state);
        ...
        if let Ok(body) = body {
            if let Ok(user_info_resp) = get_user_info(body.access_token).await {
                ...
                if let Ok(user_info) = user_info {
                    ...
                    let body = Tera::new(&(env::static_files_base_dir() + "templates/**/*")).unwrap().render("error.html", &context).unwrap();
                    return Ok(HttpResponse::Ok().body(body));
                }
            }
        }
