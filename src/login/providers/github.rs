...
impl LoginProvider for Github {
    ...
    fn login_url(&self, host: String, state: String) -> String {
        let client_id = app_env::var(env::ENV_GITHUB_APP_CLIENT_ID).expect("Missing GITHUB_APP_CLIENT_ID env var");
        let params = vec![
            ("client_id", client_id),
            ("state", state),
    ...
    async fn user_info(&self, _host: String, code: String) -> Result<ThirdPartyUserInfo, Error> {
        let client_id = app_env::var(env::ENV_GITHUB_APP_CLIENT_ID).expect("Missing GITHUB_APP_CLIENT_ID env var");
        let client_secret = app_env::var(env::ENV_GITHUB_APP_CLIENT_SECRET).expect("Missing GITHUB_APP_CLIENT_SECRET env var");
